...
class BackgroundLayer {
 ...
 public:
  ...
  virtual void combine(RgbLeds &result) const = 0;

  void publish();
};

class BackgroundLedAnimations::Solid : public BackgroundLayer {
  bool dirty = true;
...
class BackgroundLedAnimations::Xmas : public BackgroundLayer {
  const int NUM_STATES = 7;  // number of patterns to cycle through
 ...
 private:
  ...
  void loop_pre(uint8_t startTime) {
    ...
    if (pattern_ == WarmWhiteShimmer || pattern_ == RandomColorWalk) {
  ...
  void fade(unsigned char *val, unsigned char fadeTime) {
    if (*val != 0) {
      ...
      if (subAmt < 1) subAmt = 1;  // make sure we always decrease by at least 1
  ...
  void warmWhiteShimmer(unsigned char dimOnly) {
    ...
    for (int i = 0; i < LED_COUNT; i += 2) {
      ...
      // every odd LED gets set to a quarter the brighness of the preceding even
      // LED
      if (i + 1 < LED_COUNT) {
        colors[i + 1] = rgb_color(colors[i].red >> 2, colors[i].green >> 2,
                                  colors[i].blue >> 2);
      }
    }
  }
  ...
  void randomColorWalk(unsigned char initializeColors, unsigned char dimOnly) {
    ...
    for (int i = start; i < LED_COUNT; i += 7) {
      if (initializeColors == 0) {
        ...
        randomWalk(&colors[i].red, maxBrightness, changeAmount,
                   dimOnly ? 1 : 3);
        randomWalk(&colors[i].green, maxBrightness, changeAmount,
                   dimOnly ? 1 : 3);
        randomWalk(&colors[i].blue, maxBrightness, changeAmount,
                   dimOnly ? 1 : 3);
        ...
        colors[i] = rgb_color(random(maxBrightness), random(maxBrightness),
                              random(maxBrightness));
      }

      // set neighboring LEDs to be progressively dimmer versions of the color
      // we just set
      if (i >= 1) {
        colors[i - 1] = rgb_color(colors[i].red >> 2, colors[i].green >> 2,
                                  colors[i].blue >> 2);
      ...
      if (i >= 2) {
        colors[i - 2] = rgb_color(colors[i].red >> 3, colors[i].green >> 3,
                                  colors[i].blue >> 3);
      }
      if (i + 1 < LED_COUNT) {
  ...
  void traditionalColors() {
    ...
    for (unsigned int i = 0; i < extendedLEDCount; i++) {
      unsigned char brightness =
          (loopCount - initialDarkCycles) % brighteningCycles + 1;
  ...
  void colorExplosion(unsigned char noNewBursts) {
    ...
    colorExplosionColorAdjust(&colors[0].red, 9, (unsigned char *)0,
                              &colors[1].red);
    colorExplosionColorAdjust(&colors[0].green, 9, (unsigned char *)0,
                              &colors[1].green);
    colorExplosionColorAdjust(&colors[0].blue, 9, (unsigned char *)0,
                              &colors[1].blue);

    for (int i = 1; i < LED_COUNT - 1; i++) {
      // adjust the colors of second through second-to-last LEDs
      colorExplosionColorAdjust(&colors[i].red, 9, &colors[i - 1].red,
                                &colors[i + 1].red);
      colorExplosionColorAdjust(&colors[i].green, 9, &colors[i - 1].green,
                                &colors[i + 1].green);
      colorExplosionColorAdjust(&colors[i].blue, 9, &colors[i - 1].blue,
                                &colors[i + 1].blue);
    }

    // adjust the colors of the last LED
    colorExplosionColorAdjust(&colors[LED_COUNT - 1].red, 9,
                              &colors[LED_COUNT - 2].red, (unsigned char *)0);
    colorExplosionColorAdjust(&colors[LED_COUNT - 1].green, 9,
                              &colors[LED_COUNT - 2].green, (unsigned char *)0);
    colorExplosionColorAdjust(&colors[LED_COUNT - 1].blue, 9,
                              &colors[LED_COUNT - 2].blue, (unsigned char *)0);
    ...
    if (!noNewBursts) {
      ...
      for (int i = 0; i < 1; i++) {
        ...
        {
          // 2/7 chance we will spawn a red burst here (if LED has no red
          // component)
          case 0:
          case 1:
            if (colors[j].red == 0) {
          ...
          // 2/7 chance we will spawn a green burst here (if LED has no green
          // component)
          case 2:
          case 3:
            if (colors[j].green == 0) {
          ...
          case 4:
          case 5:
            if ((colors[j].red == 0) && (colors[j].green == 0) &&
                (colors[j].blue == 0)) {
              colors[j] = rgb_color(1, 1, 1);
            }
            break;

          // 1/7 chance we will spawn a blue burst here (if LED has no blue
          // component)
          case 6:
            if (colors[j].blue == 0) {
              colors[j].blue = 1;
  ...
  void gradient() {
    ...
    while (j < LED_COUNT) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        colors[(loopCount / 2 + j + LED_COUNT) % LED_COUNT] =
            rgb_color(160 - 20 * i, 20 * i, (160 - 20 * i) * 20 * i / 160);
        j++;
      }
      ...
      for (int i = 0; i < 8; i++) {
        ...
        colors[(loopCount / 2 + j + LED_COUNT) % LED_COUNT] =
            rgb_color(20 * i, 160 - 20 * i, (160 - 20 * i) * 20 * i / 160);
        j++;
      }
    }
    ...
    const unsigned char fullBrightLEDs =
        5;  // number of LEDs to leave fully bright
    ...
    unsigned int extendedLEDCount =
        (((LED_COUNT - 1) / cyclePeriod) + 1) * cyclePeriod;
  ...
  unsigned char collision() {
    ...
    const unsigned char numCollisions =
        5;                           // # of collisions before pattern ends
    ...
    if (state % 3 == 0) {
      ...
      switch (state / 3) {
        ...
        case 3:  // fourth collision: warm white streams
          colors[0] = rgb_color(maxBrightness, maxBrightness * 4 / 5,
                                maxBrightness >> 3);
          break;
        default:  // fifth collision and beyond: random-color streams
          colors[0] = rgb_color(random(maxBrightness), random(maxBrightness),
                                random(maxBrightness));
      }
      ...
      count = 8;  // pick the first value of count that results in a startIdx of
                  // 1 (see below)
      return 0;
    }
    ...
    if (state % 3 == 1) {
      ...
      if (startIdx < (LED_COUNT + 1) / 2) {
        ...
        // flash the entire strip full-brightness white (ignores maxBrightness
        // limits)
        for (int i = 0; i < LED_COUNT; i++) {
          colors[i] = rgb_color(255, 255, 255);
        }
...
class BackgroundLedAnimations::Rainbow : public BackgroundLayer {
  ...
  virtual bool update(Millis now) override {
    uint8_t time = millis() >> 4;  // divide by 16
    if (lastTime == time) {
      // nothing to do
      return false;
    }
  ...
  virtual void combine(RgbLeds &colors) const override {
    for (uint16_t i = 0; i < LED_COUNT; i++) {
      uint8_t p = lastTime - i * 8;
   ...
   ***/
  rgb_color hsvToRgb(uint16_t h, uint8_t s, uint8_t v) const {
...
namespace lighting {
extern BackgroundLedAnimations::Xmas xmas;
