...
class AnimationKeys {
 ...
 public:
  void clear(uint8_t new_idx = 0) { idx = new_idx; }

  const InflatedCmdKey &operator[](int idx) const { return cmds[idx]; }

  InflatedCmdKey &operator[](int idx) { return cmds[idx]; }

  int size() const { return idx; }

  void addAll(const AnimationKeys &keys) {
    for (int i = 0; i < keys.size(); ++i) cmds[idx++] = keys.cmds[i];
  }
...
enum class AnimatorState { Busy, Done, Final };
...
class Animator final {
 public:
  ...
  void request_stop() {
    request_stop_ = true;
    if (keysPtr == nullptr || stepperPtr == nullptr || special ||
        stepperPtr->is_ghosting()) {
      return;
    }
    if (step_mode == StepMode::CLOCKWISE ||
        step_mode == StepMode::ANTI_CLOCKWISE) {
      // actually we do not care what the intended speed was
      fill_speed_down(step_mode == StepMode::CLOCKWISE,
                      stepperPtr->turn_speed_in_revs_per_minute);
    }
  }
  ...
  void followSeconds(Micros now, bool discrete) {
    ...
    if (follow_goal < 0 || current == follow_goal) {
      ...
      if (delta > millisLeft) {
        ...
        if (discrete)
          follow_goal = Ticks::normalize((double)NUMBER_OF_STEPS *
                                         ceil(60.0 * fraction) / 60.0);
        else
          follow_goal = Ticks::normalize((double)NUMBER_OF_STEPS * fraction);
      }
    }
    if (follow_goal != current) stepperPtr->tryToStep(now);
  }
  ...
  {
    if (!speed_detection) return false;
    const auto &keys = *keysPtr;
    const auto &cur = keys[idx];
    const auto cur_speed = cmdSpeedUtil.deflate_speed(cur.inflated_speed());
    if (cur.ghost_or_alike())
  ...
  void loop(Micros now) {
    ...
    const auto speed =
        cmd.extended() ? 4 : cmdSpeedUtil.deflate_speed(cmd.inflated_speed());
...
#undef STEPS
    ...
    if (turning == 2) {
      ...
      if (special) {
        ...
        if (ghosting) {
          ...
          stepper.set_current_speed_in_revs_per_minute(clockwise ? speed
                                                                 : -speed);
        }
 ...
 public:
  void stop() { keysPtr = nullptr; }

  bool active() const { return keysPtr != nullptr; }
...
void StepExecutors::process_end_keys(int stepper_id,
  ...
  bool speed_detection0 =
      msg->speed_detection & (1 << uint64_t(stepper_id + 0));
  bool speed_detection1 =
      msg->speed_detection & (1 << uint64_t(stepper_id + 1));

  animator0.start(&animationKeysArray[0], msg->number_of_millis_left,
                  speed_detection0);
