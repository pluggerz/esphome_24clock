...
DistanceCalculators::Func DistanceCalculators::shortest = [](int from, int to) {
  bool clockwise =
      Distance::clockwise(from, to) < Distance::antiClockwise(from, to);
  return clockwise ? Distance::clockwise(from, to)
                   : -Distance::antiClockwise(from, to);
};

DistanceCalculators::Func DistanceCalculators::clockwise =
    [](int from, int to) { return Distance::clockwise(from, to); };

DistanceCalculators::Func DistanceCalculators::antiClockwise =
    [](int from, int to) { return -Distance::antiClockwise(from, to); };

void instructUsingStepCalculatorForHandle(
    Instructions &instructions, int speed, int handle_id, int to,
    const DistanceCalculators::Func &calculator) {
  ...
  auto clockwiseMode = steps > 0 ? CLOCKWISE : ANTI_CLOCKWISE;
  instructions.add(handle_id,
                   DeflatedCmdKey(clockwiseMode | ABSOLUTE, to, speed));
}
  ...
  for (int clock_id = 0; clock_id < NMBR_OF_PERFORMERS; ++clock_id) {
    ...
    if (do_swap) {
      instructUsingStepCalculatorForHandle(instructions, speed, handle0, to1,
                                           calculator);
      instructUsingStepCalculatorForHandle(instructions, speed, handle1, to0,
                                           calculator);
    } else {
      instructUsingStepCalculatorForHandle(instructions, speed, handle0, to0,
                                           calculator);
      instructUsingStepCalculatorForHandle(instructions, speed, handle1, to1,
                                           calculator);
    }
  }
}

void instructUsingSwipeWithBase(Instructions &instructions, int speed,
                                const HandlesState &goal,
                                const StepCalculator &steps_calculator,
                                int base_tick) {
  ...
  for (int handle_id = 0; handle_id < MAX_HANDLES; ++handle_id) {
    ...
    auto from = instructions[handle_id];
    max_steps_from =
        max(max_steps_from, abs(steps_calculator(from, base_tick)));
  ...
  for (int handle_id = 0; handle_id < MAX_HANDLES; ++handle_id) {
    ...
    ESP_LOGD(TAG,
             "instructUsingSwipeWithBase: handle_id=%d steps=%d ghost_steps=%d",
             handle_id, steps, ghost_steps);
    instructions.add(handle_id, DeflatedCmdKey(GHOST, ghost_steps, speed));

    // go to base_tick
    instructions.add(
        handle_id,
        DeflatedCmdKey((steps >= 0 ? CLOCKWISE : ANTI_CLOCKWISE) | RELATIVE,
                       abs(steps), speed));

    // wait a sec
    instructions.add(handle_id, DeflatedCmdKey(CLOCKWISE | RELATIVE | GHOST,
                                               100 / speed, speed));
    ...
    instructions.add(
        handle_id,
        DeflatedCmdKey(
            (additional_steps >= 0 ? CLOCKWISE : ANTI_CLOCKWISE) | RELATIVE,
            abs(additional_steps), speed));
...
class ClockIdUtil {
 public:
  ...
  static int to_row_id(int clock_id) { return (clock_id % 6) >> 1; }
...
class HandleIdUtil {
 public:
  static int to_clock_id(int handle_id) { return handle_id >> 1; };
...
void InBetweenAnimations::instructDelayUntilAllAreReady(
    Instructions &instructions, int speed, double additional_time) {
  double max_time = -1;
  for (int handle_id = 0; handle_id < MAX_HANDLES; ++handle_id) {
    if (!instructions.valid_handle(handle_id)) {
      continue;
  ...
  for (int handle_id = 0; handle_id < MAX_HANDLES; ++handle_id) {
    ...
    if (steps > 0)
      instructions.add(handle_id,
                       DeflatedCmdKey(GHOST | RELATIVE, steps, speed));
  }
}

void InBetweenAnimations::instructStarAnimation(Instructions &instructions,
                                                int speed) {
  int start_array[4] = {270, 450, 630, 90};
  ...
  HandlesAnimations::instructUsingStepCalculator(instructions, speed, start,
                                                 DistanceCalculators::shortest);
  ...
  for (int handle_id = 0; handle_id < MAX_HANDLES; ++handle_id) {
    ...
    instructions.add(
        handle_id,
        DeflatedCmdKey(RELATIVE | (clockwise ? CLOCKWISE : ANTI_CLOCKWISE),
                       steps, speed));
    instructions.add(
        handle_id,
        DeflatedCmdKey(RELATIVE | (!clockwise ? CLOCKWISE : ANTI_CLOCKWISE),
...
void InBetweenAnimations::instructPacManAnimation(Instructions &instructions,
                                                  int speed) {
  HandlesState start;
  ...
  // some delay
  instructDelayUntilAllAreReady(instructions, speed, 2.0d);
  const int randomness[2] = {NUMBER_OF_STEPS / 2 + random(NUMBER_OF_STEPS / 2),
                             NUMBER_OF_STEPS / 2 + random(NUMBER_OF_STEPS / 4)};
  instructions.iterate_handle_ids([&](int handle_id) {
    auto forward = handle_id % 2 ? CLOCKWISE : ANTI_CLOCKWISE;
    auto reverse = handle_id % 2 ? ANTI_CLOCKWISE : CLOCKWISE;
    instructions.add(
        handle_id, DeflatedCmdKey(RELATIVE | forward, NUMBER_OF_STEPS, speed));
    instructions.add(
        handle_id, DeflatedCmdKey(RELATIVE | reverse,
                                  NUMBER_OF_STEPS / 2 + randomness[0], speed));
    instructions.add(
        handle_id, DeflatedCmdKey(RELATIVE | forward,
                                  NUMBER_OF_STEPS / 2 + randomness[1], speed));
  });
}
...
auto origin_function = [](int handle_id) {
  ...
  return double(NUMBER_OF_STEPS / 4) +
         principal * double(NUMBER_OF_STEPS) / 2.0d / 3.14159265359d;
};

void InBetweenAnimations::instructAllInnerPointAnimation(
    Instructions &instructions, int speed) {
  HandlesState start;
  instructions.iterate_handle_ids([&](int handle_id) {
    start.set_ticks(handle_id,
                    origin_function(handle_id) + NUMBER_OF_STEPS / 2);
  });
  // lets go there
  HandlesAnimations::instructUsingStepCalculator(instructions, speed, start,
                                                 DistanceCalculators::shortest);
  ...
  instructions.iterate_handle_ids([&](int handle_id) {
    ...
    instructions.add(handle_id,
                     DeflatedCmdKey(RELATIVE | CLOCKWISE, steps, speed));
    instructions.add(handle_id, DeflatedCmdKey(RELATIVE | ANTI_CLOCKWISE,
                                               randomness[0], speed));
    instructions.add(
        handle_id, DeflatedCmdKey(RELATIVE | CLOCKWISE, randomness[1], speed));
  });
}

void InBetweenAnimations::instructMiddlePointAnimation(
    Instructions &instructions, int speed) {
  HandlesState start;
  instructions.iterate_handle_ids([&](int handle_id) {
...
void InBetweenAnimations::instructDashAnimation(Instructions &instructions,
                                                int speed) {
  ...
  instructions.iterate_handle_ids([&](int handle_id) {
    start.set_ticks(handle_id,
                    case_func(handle_id) == 0 ? NUMBER_OF_STEPS / 2 : 0);
  });
  // lets go there
  HandlesAnimations::instructUsingStepCalculator(instructions, speed, start,
                                                 DistanceCalculators::shortest);
  ...
  instructDelayUntilAllAreReady(instructions, speed, 2.0d);
  // our animation
  const int randomness[3] = {random(NUMBER_OF_STEPS / 4),
                             random(NUMBER_OF_STEPS / 4),
                             random(NUMBER_OF_STEPS / 4)};
  instructions.iterate_handle_ids([&](int handle_id) {
    for (auto idx = 0; idx < 3; ++idx) {
      ...
      instructions.add(handle_id, DeflatedCmdKey(RELATIVE | ANTI_CLOCKWISE,
                                                 steps * 3 / 4, speed));
      instructions.add(handle_id,
                       DeflatedCmdKey(RELATIVE | CLOCKWISE, steps / 2, speed));
    }
  });
  instructDelayUntilAllAreReady(instructions, speed, 2.0d);
}

void HandlesAnimations::instruct_using_swipe(
    Instructions &instructions, int speed, const HandlesState &goal,
    const StepCalculator &steps_calculator) {
  // find shortes path
  ...
  for (int base_tick = 0; base_tick < NUMBER_OF_STEPS; ++base_tick) {
    ...
    for (int handle_id = 0; handle_id < MAX_HANDLES; ++handle_id) {
      ...
      auto steps = abs(steps_calculator(from, base_tick)) +
                   abs(steps_calculator(base_tick, to));
    ...
    ESP_LOGVV(TAG, "For swipe: base_tick=%d -> max_steps=%d", base_tick,
              max_steps);
    if (max_steps < optimal_steps) {
      ESP_LOGVV(TAG, "For swipe: new optimal: base_tick=%d -> max_steps=%d",
                base_tick, max_steps);
